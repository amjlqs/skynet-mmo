...
local function heartbeat()
    ...
    heartbeat_timer_id = timer:add(skynet.now()+constant.HEARTBEAT_INTERVAL*100, 0, function ()
        ...
        local msg = assert(pb.encode("proto.Msg", {
            id = constant.SystemID.CMD_Heart,
            cmd = "Noop",
            payload = payload,
        }))
...
local function msg_dispatch(msg)
    ...
    local systemid = msg.id
    local req = assert(pb.decode("proto."..msg.cmd, msg.payload))
    LOG("req =", req)
    if systemid == 250 then --GM
        local pm = req.args
        util.pcall(service.client["pm_UseGm"], UID,pm)
    end
...
-- 加载玩家数据
function service.cmd.load(uid,name)
    UID = uid
    local sql = string.format("call sp_init_player(%d,%d,'%s')", uid, serverindex, name)
    --LOG("init service.cmd.load: ",sql)
...
function service.cmd.afk()
    ...
    local where_clause = string.format("actorid=%d and device_id='%s' and logout_time=0 ORDER BY login_time DESC LIMIT 1", UID, DEVICE_ID)
